...
export default function cardGrid() {
  return {
    ...
    getCardSendToSTTitle(card) {
      ...
      if (!card || !card.id) return targetName === "ST" ? "发送到 ST" : `发送到 ${targetName}`;
      if (this.isSendingToST(card.id)) {
    ...
    getReadableTagChipStyle(tag) {
      ...
      const enhanced =
        "--tag-chip-backdrop:blur(7px);--tag-chip-shadow:var(--shadow-inset),0 1px 2px var(--shadow-color);";
