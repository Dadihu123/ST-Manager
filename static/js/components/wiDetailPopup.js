...
export default function wiDetailPopup() {
  return {
        ...
        if (this.activeWiDetail.type === "embedded") {
          ...
          if (res.success) {
            ...
            this.activeWiDetail = {
              ...
              last_sent_to_tt: Number(
                res.last_sent_to_tt || this.activeWiDetail?.last_sent_to_tt || 0,
              ),
            };
    ...
    getActiveWorldInfoSentAt() {
      return this.$store?.global?.getLastSentAt?.(this.activeWiDetail) || 0;
    },
    ...
    getActiveWorldInfoSendToSTTitle() {
      ...
      if (this.isSendingWorldInfoToST) {
        return targetName === "ST" ? "正在发送到 ST" : `正在发送到 ${targetName}`;
      }
      const sentAt = this.getActiveWorldInfoSentAt();
      if (sentAt > 0) {
